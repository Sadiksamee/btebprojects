...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static function toForm($forms,$layout)
	{
		...
		if($format =='tab') $f .= '<div class="tab-content mt-4">';
		...
		{		
			...
			if($format == 'grid'  )
			{
				...
				$active = ($i==0 ? 'active' : '');
				$f .= '<div class="tab-pane p-3 '.$active.'" id="'.trim(str_replace(" ","",$tit)).'"> 
				';			
			}	
		...
		switch($type)
		{
			default;
				...
				if(isset($option['prefix']) && $option['prefix'] !='' or isset($option['sufix']) && $option['sufix'] !='')
				{
					...
					if($option['prefix'] !='')
						$form .= ' <span class="input-group-addon bg-default btn-sm">'.$option['prefix'].'</span>';
					...
					if($option['sufix'] !='')
						$form .= '<span class="input-group-addon bg-default btn-sm">'.$option['sufix'].'</span>';
				...
				if(isset($option['image_multiple']) && $option['image_multiple'] ==1)
				{
					...
					<div class="'.$field.'Upl multipleUpl">	
						<div class="custom-file mb-3 fileUpload">
			              <input type="file" id="'.$field.'" class="custom-file-input upload" name="'.$field.'[]"  '.$accept.' >
			              <label class="custom-file-label form-control" for="'.$field.'">Choose File</label>
			            </div>		
					</div>
					...
					@foreach($row[\''.$field.'\'] as $files)
						...
						<li id="cr-<?php echo $cr;?>" class="">							
							...
							<i class="ti-close  btn btn-xs btn-danger"></i></span>
					...
					$form = '
						...
						<div class="custom-file mb-3 fileUpload">
			              <input type="file" class="custom-file-input upload" name="'.$field.'"  a '.$accept.' >
			...
			case 'checkbox';
				...
				for($i=0; $i<count($opt);$i++) 
				{
					...
					<input type='checkbox' name='{$field}[]' value ='".ltrim(rtrim($row[0]))."' {$mandatory} {$attribute} class='{$extend_class} minimal-green' id='". $field."-".$i."'";
					$form .= "
	...
	static public function getNotification()
	{
		...
		SELECT 
			count(id) as total
		FROM tb_notification
		WHERE userid ='".Session('uid')."' AND is_read ='0'
		");
		if(count($total)>=1) {
			$total = $total[0]->total ;
		} else {
