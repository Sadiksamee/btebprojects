...
abstract class Controller extends BaseController {
	...
	public function getExport( Request $request, $t = 'excel')
	{
		...
		{			
			...
			return $pdf->download( $this->module.'.pdf');
	...
	function access( $id , $gid = null )
	{
		...
		if(count($row) >= 1)
		{
			...
			if($row->access_data !='')
			{
				...
				if(isset($data['is_CSV'])) {
					$data['is_csv'] = strtolower($data['is_CSV']);
				} 
				$data['is_csv'] = (isset($data['is_csv']) ? $data['is_csv'] : 0 );
				$data['is_print'] = (isset($data['is_print']) ? $data['is_print'] : 0 );
				$data['is_excel'] = (isset($data['is_excel']) ? $data['is_excel'] : 0 );
				$data['is_pdf'] = (isset($data['is_pdf']) ? $data['is_pdf'] : 0 );
			} else {
				$data = array();
			}	
