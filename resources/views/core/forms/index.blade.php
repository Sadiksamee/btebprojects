					...
					<div class="btn-group">
						...
						@endif	
				         ...
				         @if($access['is_remove'] ==1)
							 <li class="nav-item"><a href="javascript://ajax"  onclick="SximoDelete();" class="tips nav-link" title="{{ __('core.btn_remove') }}"><i class="fa fa-trash-o"></i>
		    ...
		    <table class="table  table-hover " id="{{ $pageModule }}Table">
		        <thead>
					<tr>
						...
						<th  style="width: 3% !important;"> <input type="checkbox" class="checkall" id="checkall" /><label for="checkall"></label></th>
		        ...
		        <tbody>        						
		            @foreach ($rowData as $row)
		                <tr>
							<td > {{ ++$i }} </td>
							<td ><input type="checkbox" class="ids" name="ids[]" value="{{ $row->formID }}"  id="val-{{ $row->formID }}" /> 
								<label for="val-{{ $row->formID }}"></label>
							 </td>
							<td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li class="nav-item"><a href="{{ url('core/forms/'.$row->formID.'?return='.$return)}}" class="nav-link" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li class="nav-item"><a  href="{{ url('core/forms/'.$row->formID.'/edit?return='.$return) }}" class="nav-link" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
									<li class="divider" role="separator"></li>
									@if($access['is_remove'] ==1)
										 <li class="nav-item"><a href="javascript://ajax"  onclick="SximoDelete();" class="nav-link" title="{{ __('core.btn_remove') }}">
										Remove Selected </a></li>
									@endif 
								  </ul>
								</div>
